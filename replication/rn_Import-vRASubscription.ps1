function Import-vRASubscription {
    <#
.SYNOPSIS
  Import exported VMware Automation Subscriptions
.PARAMETER Path
  The path to the json file with the Subscriptions to import. This function
  expects an array of Subscriptions.
.NOTES
  This function assumes that the Workflows used by the Subscriptions are in 
  an embedded vRO instance.

  Requires a vRAConnection variable exists. Created by New-vRAConnection.

  The Workflows that the Subscriptions use must be imported into the 
...
#>
    ...
    Process {
        ...
        foreach ($item in $jsonObject) {
            ...
            if (-not ($item.name -and $item.name.Trim() -ne "")) {
            ...
            if ($item.orgId) { 
                ...
                #TODO: Do we need to update this instead of removing it? TBC
            }
            if ($item.subscriberId) { 
                $item.PSObject.Properties.Remove('subscriberId')
            }
            ...
            $method = "POST"
            $uri = "$($vRAConnection.Server)/event-broker/api/subscriptions"
